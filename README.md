...
Let's get to an MVP quick. This'll lock in an API and DX that we can refactor behind as we hit bottlnecks or if this becomes popular. This can help make decisions in face of tradeoffs like initially using a Bun [BFF](https://blog.bitsrc.io/bff-pattern-backend-for-frontend-an-introduction-e4fa965128bf) to get an MVP of rendering out.
...
- Gleam code compiling to a JS BFF
- Eventually we want GLX (like JSX for Gleam) and CSS support, but an MVP will be quicker with pure gleam lang
- Compiles to React with Million.js
    ...
    ul([style(["display", "flex"])], [
      todos.map(fn(tdo) ->
        li([], [
          p([], [tdo.title]),
          p([], [tdo.body]),
          button([on_click(fn () { set_todos(add_todo) })], ["Done"])
        ])
      )
    ])
  ])
